...
function BannerList() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${backendUrl}/admin/listbanner?page=${page}&limit=${limit}`)
      .then((response) => {
  ...
  const deleteBanner = async (id) => {
    const confirmation = window.confirm("Are you sure you want to delete this Banner?");
    if (confirmation) {
      try {
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      const token = localStorage.getItem("token");
      axios.defaults.headers.common["Authorization"] = token;
      await axios.put(`${backendUrl}/admin/updatebannerstatus/${id}`, {
            ...
            <Card className='col-12' style={{ borderRadius: "15px" }}>
              <CardBody className='col-12'>
                <CardTitle tag="h4" className='text-dark px-5 cardtitle'>
                  <div><FontAwesomeIcon icon={faRectangleList} /> Banner List</div>
                  <Link style={{ margin: "-2% 0% 0% 0%" }} to="/admin/addbanner">
                    <Button className='btn' style={{ borderRadius: "25px", backgroundColor: "#3a7945", width: "130%", marginLeft: "-25%" }}>Add Banner</Button>
                  </Link>
                </CardTitle>
                <Table responsive>
                  <thead className="text-dark">
                    <tr>
                      ...
                      <th className='cth2'>Banner</th>
                      <th className='cth3'>Title</th>
                      <th className='cth4'>Target URL</th>
                    ...
                    {data.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className='ctd2'>
                          ...
                          <img className='courseimage' src={`${backendUrl}/images/${item.image}`} />

                        </td>
                        <td className='ctd3'>
                          <strong style={{ color: "#5ab769" }}>{item.title}</strong>
                        </td>
                        <td className='ctd4'>
                          <strong style={{ color: "#5ab769" }}></strong> {item.targeturl}
                        </td>
                        <td className='ctd5'>
                          <Switch
                        ...
                        <td className='ctd6'>
                          <button className="btn" style={{ borderRadius: "25px", backgroundColor: "#f6bf61" }} onClick={() => viewBanner(item._id)}>View</button>
                        </td>
                        <td className='ctd7'>
                          <button className="btn" style={{ borderRadius: "25px", backgroundColor: "#5ab769" }} onClick={() => EditBanner(item._id)}>Edit</button>
                        </td>
                        <td className='ctd8'>
                          <button className="btn" style={{ borderRadius: "25px", backgroundColor: "#e05757" }} onClick={() => deleteBanner(item._id)}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
